...
export async function POST(request:Request){
  ...
  try{
    ...
    if(file instanceof File&&file.size){const valid=await validateUpload(file,10*1024*1024);attachmentId=id("media");objectKey=`private/rfq/${crypto.randomUUID()}.${valid.extension}`;await putObject(objectKey,valid.bytes,valid.mime);await run("INSERT INTO media_assets(id,kind,visibility,object_key,original_name,mime_type,size_bytes,status,created_at) VALUES(?,'rfq-attachment','private',?,?,?,?,'draft',?)",attachmentId,objectKey,file.name.slice(0,180),valid.mime,valid.bytes.byteLength,now);}
    const basketId=request.headers.get("cookie")?.split(";").map((v)=>v.trim()).find((v)=>v.startsWith("yone_rfq_basket="))?.split("=")[1];
    const items=basketId?await all<{productId:string;quantity:number;name:string;sku:string|null}>('SELECT bi.product_id AS "productId",bi.quantity,p.name,p.sku FROM rfq_basket_items bi JOIN products p ON p.id=bi.product_id JOIN categories c ON c.id=p.category_id WHERE bi.basket_id=? AND p.status=\'published\' AND p.deleted_at IS NULL AND c.status=\'published\'',basketId):[];
    if(!productsText&&!items.length)throw new ValidationError("Add a product or describe the requested instruments.");
    await batch([{sql:"INSERT INTO rfqs(id,reference,full_name,email,company,country,phone,target_market,required_standard,private_label,packaging,desired_date,products_text,message,attachment_media_id,consent_at,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,'new',?,?,?)",values:[rfqId,ref,fullName,businessEmail,company,country,clean(data.get("phone"),80)||null,clean(data.get("targetMarket"),160)||null,clean(data.get("requiredStandard"),500)||null,clean(data.get("privateLabel"),3000)||null,clean(data.get("packaging"),3000)||null,clean(data.get("desiredDate"),30)||null,productsText||null,message,attachmentId,now,now,now]},...items.map((item)=>({sql:"INSERT INTO rfq_items(id,rfq_id,product_id,product_name,sku,quantity,created_at) VALUES(?,?,?,?,?,?,?)",values:[id("item"),rfqId,item.productId,item.name,item.sku,Math.max(1,Math.floor(item.quantity)),now]}))]);
  ...
  }catch(error){if(objectKey)await deleteObject(objectKey).catch(()=>undefined);if(attachmentId)await run("DELETE FROM media_assets WHERE id=?",attachmentId).catch(()=>undefined);const message=error instanceof Error?error.message:"Unable to submit the RFQ.";return NextResponse.redirect(new URL(`/rfq?error=${encodeURIComponent(message)}`,request.url),303);}
}
