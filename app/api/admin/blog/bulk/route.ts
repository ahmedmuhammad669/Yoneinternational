...
import { all, d1, first, id, unix } from "../../../../../lib/db";
...
export async function POST(request:Request){
  try{
    ...
    if(new Set(headers).size!==headers.length)throw new ValidationError("CSV header contains duplicate columns.");
    ...
    for(let index=0;index<dataRows.length;index+=1){
      const values=dataRows[index];if(values.length>headers.length)throw new ValidationError(`Row ${index+2} has more values than the header.`);
      const row=Object.fromEntries(headers.map((header,column)=>[header,values[column]?.trim()||""]));
      const title=limit(row.title,300);const body=limit(row.body,50000);if(!title||!body)throw new ValidationError(`Row ${index+2} requires both title and body.`);
      articles.push({title,slug:slug(row.slug,title),excerpt:limit(row.excerpt,1000),body,sources:limit(row.sources,5000),hero_media_id:limit(row.hero_media_id,100),author_id:limit(row.author_id,100),category_id:limit(row.category_id,100)});
    }
    const slugs=articles.map((article)=>String(article.slug));if(new Set(slugs).size!==slugs.length)throw new ValidationError("CSV contains duplicate slugs.");
    const placeholders=slugs.map(()=>"?").join(",");const existing=await all<{slug:string}>(`SELECT slug FROM blog_posts WHERE slug IN (${placeholders})`,...slugs);if(existing.length)throw new ValidationError(`Slug already exists: ${existing[0].slug}.`);
    ...
    const now=unix();await d1().batch(articles.map((article)=>d1().prepare("INSERT INTO blog_posts(id,author_id,category_id,hero_media_id,title,slug,excerpt,body,sources,updated_date,status,published_at,created_by,updated_by,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,'draft',NULL,?,?,?,?)").bind(id("post"),article.author_id,article.category_id,article.hero_media_id,article.title,article.slug,article.excerpt,article.body,article.sources,now,admin.email,admin.email,now,now)));
    await audit(admin.email,"blog.bulk_import","blog_posts",undefined,{count:articles.length,file:file.name.slice(0,180)}).catch(()=>undefined);
    return redirect(request,`${articles.length} blog draft${articles.length===1?"":"s"} imported successfully.`);
